...
with open('config.csv', newline='', encoding='UTF-8') as f:
    ...
    for row in reader:
        for url in row:
            ...
            content3 = driver.find_element_by_css_selector("#v2_contents > div.v2_contents > div > div.v2_gridC.v2_section.v2_clear.v2_stationUnkouMap.v3_stationUnkouMap > div.v2_sectionS.v2_gridCRow > div.v2_unkouReportInfo > div > p")

            # lineに通知したいメッセージを入力
            content.append("●" + content1.text[:-5])
            content.append(content2.text)
            content.append(content3.text)
